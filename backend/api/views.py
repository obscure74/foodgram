...
class UserViewSet(viewsets.ModelViewSet):
    ...
    def subscribe(self, request, pk=None):
        ...
        if request.method == 'POST':
            ...
            return Response(SubscriptionSerializer(author, context={'request': request}).data, status=status.HTTP_201_CREATED)
    ...
    def subscriptions(self, request):
        subscribed_authors = User.objects.filter(subscribers__user=request.user)
    ...
    @action(detail=False, methods=['get', 'put', 'delete'], permission_classes=[IsAuthenticated], url_path='me')
    def me(self, request):
        if request.method == 'GET':
            return Response(CustomUserSerializer(request.user, context={'request': request}).data)
        elif request.method == 'PUT':
            serializer = CustomUserSerializer(request.user, data=request.data, context={'request': request})
...
class RecipeViewSet(viewsets.ModelViewSet):
    ...
    def download_shopping_cart(self, request):
        shopping_recipes = ShoppingCart.objects.filter(user=request.user).values_list("recipe_id", flat=True)
        ...
        for item in ingredients:
            content.append(f"{item['ingredient__name']} ({item['ingredient__measurement_unit']}) — {item['total_amount']}")
